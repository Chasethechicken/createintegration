...
public class CopperPressurePlate extends WeightedPressurePlateBlock {
    ...
    public void randomTick(@Nonnull BlockState state, ServerWorld worldIn, @Nonnull BlockPos pos, @Nonnull Random random) {
        if (worldIn.getRandom().nextFloat() <= 1 / 32f) {
            ...
            for (Direction facing : Direction.values()) {
                ...
                if (neighborState.getProperties().contains(OXIDIZATION) && neighborState.get(OXIDIZATION) != 0) {
                    neighbors.add(neighborState.get(OXIDIZATION));
                }
                if (Block.hasEnoughSolidSide(worldIn, neighbourPos, facing.getOpposite())) {
                    continue;
                }
                canIncrease = true;
            }
    ...
    /* @Override // FIXME
    public float getBlockHardness(@Nonnull IBlockReader worldIn, @Nonnull BlockPos pos) {
